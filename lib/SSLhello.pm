...
my  $SID_sslhelo= "@(#) SSLhello.pm 3.3 24/01/11 23:05:22";
...
$SSLhello::max_ciphers          = _MY_SSL3_MAX_CIPHERS; # max nr of ciphers sent in a SSL3/TLS Client-Hello to test if they are supported by the server
...
sub checkSSLciphers ($$$@) {
    ...
    if ($protocol == $PROTOCOL_VERSION{'SSLv2'}) { #SSL2
        ...
        foreach my $cipher_str (@cipher_str_array) {
            ...
            if ( $arrayLen >= $maxCiphers) { # test up to ... ciphers ($SSLhello::max_ciphers = _MY_SSL3_MAX_CIPHERS) with 1 doCheckSSLciphers (=> Client Hello)
        ...
        if ( ($SSLhello::proxyhost) && ($SSLhello::proxyport) ) { # via proxy
            ...
            { # >> start a block
                ...
                eval {
                    ...
                    close ($socket) or carp("**WARNING: ". error_handler->get_err_str() ."; Can't close socket, too: $!"); #tbd löschen ###
            ...
            { # >> start a block
                ...
                eval {
                    ...
                    close ($socket) or carp("**WARNING: ". error_handler->get_err_str() ."; Can't close socket, too: $!");
                        ...
                        FOREACH_NEW_VALUE: foreach my $_new_ele (@{$_param_hash{values}}) { # all new values or arrys of values
                            ...
                            FOREACH_STORED_VALUE: foreach my $_stored_ele (@{$_param_hash_ref->{$_extension}{$_ext_ch_rx}{values}}) { # all stored values or arrys of values
                                ...
                                if ( (ref (\$_new_ele) eq "SCALAR") && (ref (\$_stored_ele) eq "SCALAR") ) {        # both arrays are an simple array
                                    ...
                                    carp ("**WARNING: SSLhello::_parseExtensions: internal error in result hash: can't compare and store new values: "
...
##          && ((@{$SSLhello::{param}{$_extension}{RX}{format_positions}}) == 1)) { # only one parameter array ([0][x]) as default and only one dimensional result array ### }
            ...
            carp ("**WARNING: SSLhello::_doCheckAllExtensions ($_extension): Detailled checks for extensions with multiple parameters are not supported, yet. Please check variable '\$SSLhello::check_extensions').");
        }
...
#                       return ("", $lastMsgType, 0,"", parseTLS_ServerHello ($host, $port, $message, $serverHello{'msg_len'},$client_protocol) ); # moved bebind the 'while-loop'
                ...
                carp ("**WARNING: parseHandshakeRecord: Server '$host:$port': Unknown SSL/TLS record type received that is not (yet) defined in SSLhello.pm:\n");
