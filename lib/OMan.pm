...
my  $SID_oman   = "@(#) OMan.pm 3.24 24/03/28 12:15:36";
...
sub _man_get_version {
    ...
    my $v = '3.24'; $v = _VERSION() if (defined &_VERSION);
    return $v;
} # _man_get_version
...
sub _man_pod_file   {
    #? print file content in POD format
    my $file = shift;
    return "" if (not -e $file);
    @help = ODoc::get_markup($file, __PACKAGE__, $VERSION);
    my $txt = "#!/usr/bin/perldoc\n#\n# Generated by __FILE__\n\n";
       $txt .= "=pod\n\n";         # hack to fool Perl's own POD parser
       $txt .= "=encoding utf8\n\n";
    return $txt . _man_pod_text();
} # _man_pod_file
...
sub _oman_main      {
    ...
    while (my $arg = shift @ARGV) {
        ...
        if ($arg =~ /--pod=(.*)$/)       { print _man_pod_file($1); next; }
