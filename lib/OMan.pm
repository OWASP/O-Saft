...
sub _man_cmd_from_source {
    ...
    my $skip = 1;
    my $fh   = undef;
    _man_dbx("_man_cmd_from_source: reading lib/OData.pm");
...
sub man_warnings    {
    ...
    if (0 < (grep{/^--v/} @ARGV)) {     # **INFO in main
        printf("%s%s\n", $STR{'INFO'}||'**INFO: ', "  man_warnings: reading $doc");
    }
...
sub man_src_grep    {
    ...
    my $fh  = undef;
    my $src = _get_filename($parent);   # need full path for $parent file here
    _man_dbx("man_src_grep: reading $src");
    if (open($fh, '<:encoding(UTF-8)', $src)) {
        while(<$fh>) {
            next if (m(^\s*#));
