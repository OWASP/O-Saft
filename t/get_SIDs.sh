...
#
#? VERSION
#?      @(#) get_SIDs.sh 1.2 24/08/14 10:33:17
#?
#? AUTHOR
...
(
  \gawk '
	...
	/@\(#)/ {
		...
		if ($2!=f && $2!="get_SIDs.sh") {next;}
		   # ignore if filename does not match path, but allow get_SIDs.sh
...
) | \sort -f -k 5 | \sed -e '/^..WARNING:/s/SID/duplicate SID found in/'
