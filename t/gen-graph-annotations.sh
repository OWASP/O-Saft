...
#TODO:  neet to search and santise L<anotation>
	  for _src in $ALL_sort ; do
		...
		\perl -lne '
			...
			if ($ARGV =~ m#^doc/*#)     { $c = "{color:#00ff00;}"; } # green
			if ($ARGV =~ m#^o-saft*#)   { $c = "{color:#ff9900;}"; } # gold
			if ($ARGV =~ m#^lib/*#)     { $c = "{color:#ff9900;}"; }
			if ($ARGV =~ m#^usr/*#)     { $c = "{color:#dda0dd;}"; } # plum
	  ...
	  for _src in $ALL_nopod ; do
        ...
        \echo "# generated by gen-graph-annotations.sh 3.2" >>$_out.graph
...
#!/usr/bin/wish
# generated by gen-graph-annotations.sh 3.2
