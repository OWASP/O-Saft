...
@(#) Makefile.pod 1.32 19/11/20 09:18:45
...
Following examples provide help (documentation) for the available targets:
...
To just get the list of all targets following can be used (example):
...
their functionality is mainly described herein.
...
* target names only use lowercase characters and . and - and _
...
The main Makefile is an exception, because it uses commonly used names.
...
hoever, huge definitions are splited in multiple lines  for better (human)
readability.
...
Targets in the makefiles are grouped, usually. Each group should be headed
...
Example:
    ...
    HELP.typ        = $(_NL)\
    \# same addition text with more than one line $(_NL)\
    \# other line
...
these lines is printed as follows:
    ...
    # same addition text with more than one line
    # other line
...
Remember important variables:
    ...
    $(MAKEFILE_LIST)- inclueded Makefiles in the order they have been read
...
Unfortunatelly this  results in  untested commands,  those defined  in the
