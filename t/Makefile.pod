...
@(#) Makefile.pod 1.26 19/11/09 21:21:36
...
The macro  _ignore-output ,which is based on _ignore-output-keys) contains
these keys (commands).  This macro should be used whenever such a command
is tested.

Unfortunatelly this results in untested commands,  those defined in  macro
_ignore-output-keys . To test these commands also, a special target should
exist.
