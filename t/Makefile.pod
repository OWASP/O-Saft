...
@(#) Makefile.pod 1.23 19/10/18 23:47:12
...
General rules for our variable names in Makefiles are:

    * variable and target names consist only of characters a-zA-Z0-9_.
    * variable and target names start with upper case letters or _
    * variable and target names starting _ are intended for internal use
...
it is used verbatime to ensure that exactly this string is used and cannot
be overwritten (i.e. with an environment variable) when make is called. 
If a Makefile is changed, all occourances of the string must be changed.
