...
For more details on target names, please  SEE O-Saft Makefile Targets .
...
@(#) Makefile.pod 1.54 22/11/10 22:11:05
...
Sometimes Make's  'define'  are used instead of variables (macros), mainly
...
Following name prefixes are used for variables:
    ...
    SRC.contrib     - something related to the contrib/ directory
...
It's silently ignored if the file required by the  --include  directive is
missing.
...
Therefor, GNU Make provides "user defined functions"  which are defined in
Makefile.gen.
...
General functions:
    ...
    $(call test-targets,testcmd,TYP,TOOL,VAR-name,TEST.args,TEST.init)
    $(call test-targets,testcmd,TYP,TOOL,VAR-name,TEST.init,TEST.args)

Wrapper functions (calling the general functions):

    $(call test-targets-args,testarg,TYP,TOOL,VAR-name)
    $(call test-targets-init,testarg,TYP,TOOL,VAR-name)
    $(call test-targets-args,testcmd,TYP,TOOL,VAR-name)
    $(call test-targets-init,testcmd,TYP,TOOL,VAR-name)
...
added to the corresponding / well-known macros, for example:  ALL.*  etc..
...
=item Generate targets for various options
    ...
    $(call test-targets-args,testarg,typ,$(TOOL),LIST.tool)

This generates following targets and macros:
...
=item Generate targets for various options
    ...
    $(call test-targets-init,testarg,typ,$(TOOL),LIST.tool)
