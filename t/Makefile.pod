...
the variable  O-SID or O-SID.*  should be used instead for all usages. But
...
The value of the O-SID* variables should be managed by the version control
system.
...
Following internal variables in each Makefile* are used:
    ...
    O-SID           - version in project's Makefile
    O-SID.*         - version in included Makefile*
    O-SELF.*        - path of the Makefile itself (in included Makefile*)

The  O-SID.*  variables are used to check if sub-makefiles were included.
...
* The variable with the prefix  HELP. for example  HELP.help,  is used for
  ...
  each line must be terminated with $(O-NL)\  which will be evaluated when
...
Example:
    ...
    HELP.typ        = $(O-NL)\
    \# some additional text with more than one line $(O-NL)\
    \# other line
...
be checked before including. The private  O-SID.*  variable is used, which
...
Target names may occur as following constructs:
...
variable's value, $(O-NL) is used for that.

GNU Make's define directive, which would avoid the use of  $(O-NL), is not
...
configured in with the  OSAFT_OPTIONS environment variable.  This enforces
printing of the command line at beginning of output.
Because this option is part of  all calls, it is not passed as argument by
the (make-)variables used by the targets, but the environment variable set
in Makefile. This way, it does not disturb the Makefile* and it's also not
printed in the comaqnd line (which itself is enforced by  --trace-CLI ).
