...
#
#? VERSION
#?      @(#) Makefile.pod 1.8 19/03/14 23:30:37
#?
#? AUTHOR
...
Traditionally  $(MAKE) is used for the make command itself. For conveience
...
=head2 Make:collecting targets

Target names are collected in variables. GNU make's  $(shell ...)  is used
for that, which dynamically extracts the target names from the Makefile.
...
The variables should contain real (static) targets only, no pattern rules.
