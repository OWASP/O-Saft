...
#.        opens the document in the application specified (here, TextEdit).
#.      - TODO (8/2016):
...
proc pinfo        {txt} {
    ...
    if {![regexp -- {--(d|v|dbx|debug)$} $argv]} { return }
        # $cfg(VERB) may not yet set, hence cannot be used
...
# + or -
set cfg(SID)    "@(#) o-saft.tcl 3.38 25/02/26 00:37:24"
set cfg(mySID)  "$cfg(SID) Spring Edition 2025"
                 # contribution to SCCS's "what" to avoid additional characters
set cfg(VERSION) {3.38}
...
#  without --trace-CLI it would be on line with the version number like:
#   24.06.24
...
catch { exec {*}$prg(PERL) $prg(SAFT) +VERSION } usage error_opts
#_dbx 3 " error_opts=$error_opts"
if {0!=[dict get $error_opts -code]} { _message error "**ERROR: $prg(SAFT) failed" $usage }
if {![regexp {^\d\d\.\d\d\.\d\d} $usage]} { # check other PATH
    ...
    if {![regexp -line {^\d\d\.\d\d\.\d\d} $usage]} {
...
proc self_write_rc      {}  {
 ...
 #?
 #? VERSION
 #?      @(#) .o-saft.tcl generated by 3.38 25/02/26 00:37:24
 #?
 #? AUTHOR
...
proc osaft_read_data {norc mode} {
    ...
    #_dbx 3 " error_opts=$error_opts"
    if {0!=[dict get $error_opts -code]} { _message error "**ERROR: $prg(SAFT) failed" $txt }
    _dbx 2 "=... RC text ... }"
    return $txt
}; # osaft_read_data
...
proc gui_init:prg {start} {
    ...
    # $start required for Windows and Aqua, otherwise empty
    _dbx 2 "{}{"
    global prg
## 23feb25 Liste in global prg definieren
    foreach bin " $start \
            firefox chrome chromium iceweasel konqueror mozilla \
            netscape opera safari webkit htmlview www-browser w3m" {
        if {[regexp  {^\s*$}  $bin]} { continue }  ;# skip empty values
...
# FIXME: if {[_isaqua]} ...
    if {"O-Saft" ne $prg(TKPOD)} {
        ...
        if {[string length    $binary]} {
            ...
            _message warning "--pod" "no $prg(TKPOD) found; using built-in viewer"
...
proc gui_init:geo   {}  {
    ...
    if {[_isaqua]}         { set myX(miny) 820  };# because fonts are bigger by default
