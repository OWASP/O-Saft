...
# + or -
set cfg(SID)    "@(#) o-saft.tcl 3.28 24/08/13 14:09:05"
set cfg(mySID)  "$cfg(SID) Summer Edition 2024"
                 # contribution to SCCS's "what" to avoid additional characters
set cfg(VERSION) {3.28}
...
proc osaft_write_rc     {}  {
 ...
 #?
 #? VERSION
 #?      @(#) .o-saft.tcl generated by 3.28 24/08/13 14:09:05
 #?
 #? AUTHOR
    ...
    foreach mode $cfg(docs-help) {
        ...
        if {2 < [llength [split $txt "\n"]]} {
            ...
            switch $key {
              ...
              {OSAFT_MAKE} {
                          putv " previous line for testing only, should be same as next line:"
                          putv "'exec {*} o-saft.pl  --no-rc --help=OSAFT_MAKE'"
                        }
...
proc osaft_exec   {parent cmd}  {
    ...
    if {[regexp {[+]info}     $do]}  { set _layout "text" };# no table data
...
foreach {idx val} [array get exe] { set cfg($idx) $val; puts "cfg($idx)" }
