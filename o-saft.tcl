...
# + or -
set cfg(SID)    "@(#) o-saft.tcl 3.45 25/02/26 13:20:33"
set cfg(mySID)  "$cfg(SID) Spring Edition 2025"
                 # contribution to SCCS's "what" to avoid additional characters
set cfg(VERSION) {3.45}
...
#  without --trace-CLI it would be one line with the version number like:
#   24.06.24
...
if {0!=[dict get $error_opts -code]} { _message error "$prg(SAFT) failed" $usage }
...
proc self_write_rc      {}  {
 ...
 #?
 #? VERSION
 #?      @(#) .o-saft.tcl generated by 3.45 25/02/26 13:20:33
 #?
 #? AUTHOR
...
proc osaft_read_data {norc mode} {
    ...
    if {"file" eq $cfg(docs-src)} {
        ...
        if {"" ne $fid} {
            ...
            if {1==[info exists env(ANDROID_DATA)]} {
                _message warning "(osaft_read_data)" \
    ...
    if {0!=[dict get $error_opts -code]} {
        _message error "(osaft_read_data)" "$prg(SAFT) failed\n----\n$txt"
    }
    _dbx 2 "=... RC text ... }"
    return $txt
}; # osaft_read_data
...
# FIXME: if {[_isaqua]} ...
    if {"O-Saft" ne $prg(TKPOD)} {
        ...
        if {[string length    $binary]} {
            ...
            _message warning "(gui_init:prg): --pod" "no $prg(TKPOD) found; using built-in viewer"
