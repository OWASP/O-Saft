...
# + or -
set cfg(SID)    "@(#) o-saft.tcl 3.48 25/02/27 09:34:57"
set cfg(mySID)  "$cfg(SID) Spring Edition 2025"
                 # contribution to SCCS's "what" to avoid additional characters
set cfg(VERSION) {3.48}
...
proc self_write_rc      {}  {
 ...
 #?
 #? VERSION
 #?      @(#) .o-saft.tcl generated by 3.48 25/02/27 09:34:57
 #?
 #? AUTHOR
...
proc config_osaft {}    {
    ...
    if {0==[_isempty  $_perl]} {
        if {"$prg(PERL)"!="$_perl"} { _message warning "(config_osaft)" "using: $_perl $osaft ..." }
...
----
!!Hint:
check PATH environment variable, or use --perl=FILE option."
