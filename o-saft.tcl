...
#?
#?      The option  --header  is added to all  +cipher*  commands.  It is used
#?      to get the  "Ciphers: Checking ..."  header line in  the table view of
#?      the results.  Otherwise the reported cipher suites could not be mapped
#?      to the proper SSL/TLS protocol.
#?
...
set cfg(SID)    "@(#) o-saft.tcl 1.267 22/04/01 20:44:31"
set cfg(mySID)  "$cfg(SID) Spring Edition 2021"
                 # contribution to SCCS's "what" to avoid additional characters
set cfg(VERSION) {1.267}
...
proc create_host  {parent host_nr}  {
    ...
    if {0==$host_nr} {
        ...
        incr hosts(0)
        set  host_nr $hosts(0) ;# now host_nr > 0
        set  hosts($host_nr) ""
    }
    frame        $this
    grid [label  $this.lh -text [_get_text host]] \
         ...
         [button $this.host_add -command "create_host {$parent} 0;"] \
...
proc osaft_write_rc     {}  {
 ...
 #?
 #? VERSION
 #?      @(#) .o-saft.tcl generated by 1.267 22/04/01 20:44:31
 #?
 #? AUTHOR
...
proc osaft_write_data   {}  {
    ...
    _dbx 2 "{}"
    global cfg prg
    foreach key [list commands opt alias data checks regex rfc glossar help] {
        set mode +help     ;# default
        if {"help" ne "$key"} {
            set mode --help=$key
        }
        set file [_get_data_file  $mode]
        if [catch { set fid [open $file w]} err] { puts "**ERROR: $err; file ignored"; continue }
        putv  " exec {*}$prg(PERL) $prg(SAFT) [docker_args] --no-rc $mode"
        catch { exec {*}$prg(PERL) $prg(SAFT) [docker_args] --no-rc $mode } txt
        putv  " write $file"
        puts $fid "$txt"
        close $fid
    }
...
proc osaft_exec   {parent cmd}  {
    ...
    # quick&dirty hack to get headline in table view;# FIXME: see KNOWN PROBLEMS
    if {[regexp {[+]cipher} $do]}   { lappend opt "--header" }
