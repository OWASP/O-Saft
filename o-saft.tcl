...
# + or -
set cfg(SID)    "@(#) o-saft.tcl 3.33 25/02/24 10:47:36"
set cfg(mySID)  "$cfg(SID) Spring Edition 2025"
                 # contribution to SCCS's "what" to avoid additional characters
set cfg(VERSION) {3.33}
...
proc osaft_write_rc     {}  {
 ...
 #?
 #? VERSION
 #?      @(#) .o-saft.tcl generated by 3.33 25/02/24 10:47:36
 #?
 #? AUTHOR
...
# TODO: on Mac OS X add: set version [exec sw_vers -productVersion]
    ...
    set sys "env(...)"
    foreach var [list HOME SHELL USER DISPLAY LANG PATH ANDROID_DATA \
...
puts "$prg(TKPOD) :: binary=$binary :: [string length   $binary]"
#exit
        _dbx 4 " viewer= $bin $binary"
        if {[string length   $binary]} {
            set prg(TKPOD)   $binary
        } else {
            _message warning "--pod" \
                "no $prg(TKPOD) found; using built-in viewer"
...
proc gui_init:geo   {}  {
    ...
    if {[_isaqua]}         { set myX(miny) 770  };# because fonts are bigger by default
