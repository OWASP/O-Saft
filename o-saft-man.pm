...
sub _man_cmd_from_source {
    ...
    my $skip = 1;
    my $fh   = undef;
    _man_dbx("_man_cmd_from_source: OSaft/Data.pm");
    if (open($fh, '<:encoding(UTF-8)', _get_filename("OSaft/Data.pm"))) { # file must be hardcoded here
        while(<$fh>) {
...
sub printhelp       {   ## no critic qw(Subroutines::ProhibitExcessComplexity)
    ...
    if ($hlp =~ m/^(devel|developer|development)$/i) { # print developer description
        $txt = join("", OSaft::Doc::Data::get("devel.txt", $parent, $version));
    }
