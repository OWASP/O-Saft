...
#?
#? VERSION
#?      @(#) Makefile 1.86 19/11/21 09:07:19
#?
#? AUTHOR
...
_INST.text      = generated from Makefile 1.86
...
# The perl script takes care of them and pretty prints the strings.
...
$(GEN.rel): $(ALL.src)
	sccs what $(ALL.src) | \
	perl -anle '/^(.*):$$/&&do{$$f=$$m=$$1;$$m=~s#.*/##;next;};/.*?($$m|%[M]%)/&&do{$$f=~s/yeast/o-saft/;$$F[0]=~s/yeast/o-saft/;$$t=$$F[3];$$q=chr(0x27);$$t=~s#["$$q,]##g;printf("%s\t%s\t%s\t%s\t%s\n",$$F[1],$$F[2],$$t,$$F[0],$$f)};'
rel :$(GEN.rel)
